...
export class ProductsComponent implements OnInit {
  ...
  remove(product: Product): void {
    const line_count = product._count?.assemblyLines ?? 0;
    const warning =
      line_count > 0
        ? `"${product.name}" has ${line_count} assembly line(s). Deleting it will also delete them and their allocations. Continue?`
        : `Delete "${product.name}"?`;
